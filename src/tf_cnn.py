...
def model(X_train, Y_train, X_test, Y_test, learning_rate=0.009,
    ...
    # Get all variables
    model_vars = tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.GLOBAL_VARIABLES)
    ...
    with tf.compat.v1.Session() as sess:
        ...
        plt.title(
            "Learning rate = {0}, Train Accuracy: {1}, Test Accuracy: {2}.".format(learning_rate, train_accuracy, test_accuracy))
        plt.show()
...
def run_model(
    ...
    _, _, parameters, save_path = model(X_train, Y_train, X_test, Y_test, learning_rate, num_epochs, minibatch_size, print_cost)
